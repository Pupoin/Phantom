...
void DicomDetectorConstruction::InitialisationOfMaterials()
{
    ...
    // add by Zhaoyang Yuan, 2022.1.12 10:33
    //
    // resin

    G4Material* resin = new G4Material ("Resin", 1.15*g/cm3, 
                                             numberofElements = 3);
    resin->AddElement(elC, 11);
    resin->AddElement(elH, 12);
    resin->AddElement(elO, 3);


    // this is based on paper 10.1093/jicru/os24.1.Report46
    // all use the first one in the paper.
    G4Material* adipose_tissue = new G4Material ("Adipose_tissue", 1.00*g/cm3, 
                                             numberofElements = 7);
    adipose_tissue->AddElement(elH, 11.1*perCent);
    adipose_tissue->AddElement(elC, 20.5*perCent);
    adipose_tissue->AddElement(elN, 0.9 *perCent);
    adipose_tissue->AddElement(elO, 67.2*perCent);
    adipose_tissue->AddElement(elNa, 0.1*perCent);
    adipose_tissue->AddElement(elCl, 0.1*perCent);
    adipose_tissue->AddElement(elS, 0.1*perCent);

    G4Material* blood_whole = new G4Material ("Blood_whole", 1.04*g/cm3, 
                                             numberofElements = 10);
    blood_whole->AddElement(elH, 10.5*perCent);
    blood_whole->AddElement(elC, 7.3*perCent);
    blood_whole->AddElement(elN, 2.2 *perCent);
    blood_whole->AddElement(elO, 79.2*perCent);
    blood_whole->AddElement(elNa, 0.1*perCent);
    blood_whole->AddElement(elP, 0.1*perCent);
    blood_whole->AddElement(elS, 0.1*perCent);
    blood_whole->AddElement(elCl, 0.2*perCent);
    blood_whole->AddElement(elK, 0.2*perCent);
    blood_whole->AddElement(elFe, 0.1*perCent);

    G4Material* brain_whole = new G4Material ("Brain_whole", 1.02*g/cm3, 
                                             numberofElements = 8);
    brain_whole->AddElement(elH, 10.9*perCent);
    brain_whole->AddElement(elC, 3.3*perCent);
    brain_whole->AddElement(elN, 0.7 *perCent);
    brain_whole->AddElement(elO, 84.2*perCent);
    brain_whole->AddElement(elNa, 0.2*perCent);
    brain_whole->AddElement(elP, 0.2*perCent);
    brain_whole->AddElement(elCl, 0.3*perCent);
    brain_whole->AddElement(elK, 0.2*perCent);

    G4Material* breast_calcifications = new G4Material ("Breast_calcifications", 3.06*g/cm3, 
                                             numberofElements = 6);
    breast_calcifications->AddElement(elH, 0.3*perCent);
    breast_calcifications->AddElement(elC, 1.6*perCent);
    breast_calcifications->AddElement(elN, 0.5 *perCent);
    breast_calcifications->AddElement(elO, 40.7*perCent);
    breast_calcifications->AddElement(elP, 18.7*perCent);
    breast_calcifications->AddElement(elCa, 38.2*perCent);

    G4Material* breast_mammary_gland = new G4Material ("Breast_mammary_gland", 0.99*g/cm3, 
                                             numberofElements = 8);
    breast_mammary_gland->AddElement(elH, 10.9*perCent);
    breast_mammary_gland->AddElement(elC, 50.6*perCent);
    breast_mammary_gland->AddElement(elN, 2.3 *perCent);
    breast_mammary_gland->AddElement(elO, 35.8*perCent);
    breast_mammary_gland->AddElement(elNa, 0.1*perCent);
    breast_mammary_gland->AddElement(elP, 0.1*perCent);
    breast_mammary_gland->AddElement(elS, 0.1*perCent);
    breast_mammary_gland->AddElement(elCl, 0.2*perCent);

    G4Material* eye_lens = new G4Material ("Eye_lens", 1.07*g/cm3, 
                                             numberofElements = 8);
    eye_lens->AddElement(elH, 9.6*perCent);
    eye_lens->AddElement(elC, 19.5*perCent);
    eye_lens->AddElement(elN, 5.7 *perCent);
    eye_lens->AddElement(elO, 64.6*perCent);
    eye_lens->AddElement(elNa, 0.1*perCent);
    eye_lens->AddElement(elP, 0.1*perCent);
    eye_lens->AddElement(elS, 0.3*perCent);
    eye_lens->AddElement(elCl, 0.1*perCent);

    G4Material* gI_tract_intestine = new G4Material ("GI_tract_intestine", 1.03*g/cm3, 
                                             numberofElements = 9);
    gI_tract_intestine->AddElement(elH, 10.6*perCent);
    gI_tract_intestine->AddElement(elC, 11.5*perCent);
    gI_tract_intestine->AddElement(elN, 2.2 *perCent);
    gI_tract_intestine->AddElement(elO, 75.1*perCent);
    gI_tract_intestine->AddElement(elNa, 0.1*perCent);
    gI_tract_intestine->AddElement(elP, 0.1*perCent);
    gI_tract_intestine->AddElement(elS, 0.1*perCent);
    gI_tract_intestine->AddElement(elCl, 0.2*perCent);
    gI_tract_intestine->AddElement(elK, 0.1*perCent);


    G4Material* heart = new G4Material ("Heart", 1.04*g/cm3, 
                                             numberofElements = 9);
    heart->AddElement(elH, 10.6*perCent);
    heart->AddElement(elC, 7.5*perCent);
    heart->AddElement(elN, 1.8 *perCent);
    heart->AddElement(elO, 79.3*perCent);
    heart->AddElement(elNa, 0.2*perCent);
    heart->AddElement(elP, 0.1*perCent);
    heart->AddElement(elS, 0.1*perCent);
    heart->AddElement(elCl, 0.2*perCent);
    heart->AddElement(elK, 0.2*perCent);

    G4Material* heart_blood_filled = new G4Material ("Heart_blood_filled", 1.06*g/cm3, 
                                             numberofElements = 10);
    heart_blood_filled->AddElement(elH, 10.3*perCent);
    heart_blood_filled->AddElement(elC, 12.1*perCent);
    heart_blood_filled->AddElement(elN, 3.2 *perCent);
    heart_blood_filled->AddElement(elO, 73.4*perCent);
    heart_blood_filled->AddElement(elNa, 0.1*perCent);
    heart_blood_filled->AddElement(elP, 0.1*perCent);
    heart_blood_filled->AddElement(elS, 0.2*perCent);
    heart_blood_filled->AddElement(elCl, 0.3*perCent);
    heart_blood_filled->AddElement(elK, 0.2*perCent);
    heart_blood_filled->AddElement(elFe, 0.1*perCent);

    G4Material* kidney = new G4Material ("Kidney", 1.03*g/cm3, 
                                             numberofElements = 9);
    kidney->AddElement(elH, 10.7*perCent);
    kidney->AddElement(elC, 6.4*perCent);
    kidney->AddElement(elN, 1.6 *perCent);
    kidney->AddElement(elO, 80.4*perCent);
    kidney->AddElement(elNa, 0.2*perCent);
    kidney->AddElement(elP, 0.2*perCent);
    kidney->AddElement(elS, 0.1*perCent);
    kidney->AddElement(elCl, 0.2*perCent);
    kidney->AddElement(elK, 0.2*perCent);
    
    G4Material* liver = new G4Material( "Liver",
                                       density = 1.04*g/cm3,
                                       numberofElements = 9);
    liver->AddElement(elH, 10.5*perCent);
    liver->AddElement(elC, 9.1*perCent);
    liver->AddElement(elN, 2.1*perCent);
    liver->AddElement(elO, 77.4*perCent);
    liver->AddElement(elNa, 0.2*perCent);
    liver->AddElement(elP, 0.2*perCent);
    liver->AddElement(elS, 0.1*perCent);
    liver->AddElement(elCl, 0.2*perCent);
    liver->AddElement(elK, 0.2*perCent);    
    ...
    G4Material* lung = new G4Material( "Lung",
                                       density = 1.04*g/cm3,
                                       numberofElements = 9);
    lung->AddElement(elH, 10.6*perCent);
    lung->AddElement(elC, 7.6*perCent);
    lung->AddElement(elN, 1.8*perCent);
    lung->AddElement(elO, 79.2*perCent);
    lung->AddElement(elNa, 0.2*perCent);
    lung->AddElement(elP, 0.2*perCent);
    lung->AddElement(elS, 0.1*perCent);
    lung->AddElement(elCl, 0.2*perCent);
    lung->AddElement(elK, 0.2*perCent);    
    
    G4Material* lymph = new G4Material( "Lymph",
                                       density = 1.03*g/cm3,
                                       numberofElements = 7);
    lymph->AddElement(elH, 10.8*perCent);
    lymph->AddElement(elC, 4.1*perCent);
    lymph->AddElement(elN, 1.1*perCent);
    lymph->AddElement(elO, 83.2*perCent);
    lymph->AddElement(elNa, 0.3*perCent);
    lymph->AddElement(elS, 0.1*perCent);
    lymph->AddElement(elCl, 0.4*perCent);

    G4Material* muscle_skeletal = new G4Material( "Muscle_skeletal",
                                       density = 1.03*g/cm3,
                                       numberofElements = 9);
    muscle_skeletal->AddElement(elH, 10.8*perCent);
    muscle_skeletal->AddElement(elC, 4.9*perCent);
    muscle_skeletal->AddElement(elN, 1.3*perCent);
    muscle_skeletal->AddElement(elO, 82.1*perCent);
    muscle_skeletal->AddElement(elNa, 0.2*perCent);
    muscle_skeletal->AddElement(elP, 0.1*perCent);
    muscle_skeletal->AddElement(elS, 0.1*perCent);
    muscle_skeletal->AddElement(elCl, 0.3*perCent);
    muscle_skeletal->AddElement(elK, 0.2*perCent);  

    G4Material* ovary = new G4Material( "Ovary",
                                       density = 1.05*g/cm3,
                                       numberofElements = 9);
    ovary->AddElement(elH, 10.5*perCent);
    ovary->AddElement(elC, 9.3*perCent);
    ovary->AddElement(elN, 2.4*perCent);
    ovary->AddElement(elO, 76.8*perCent);
    ovary->AddElement(elNa, 0.2*perCent);
    ovary->AddElement(elP, 0.2*perCent);
    ovary->AddElement(elS, 0.2*perCent);
    ovary->AddElement(elCl, 0.2*perCent);
    ovary->AddElement(elK, 0.2*perCent);  

    G4Material* pancreas = new G4Material( "Pancreas",
                                       density = 1.04*g/cm3,
                                       numberofElements = 9);
    pancreas->AddElement(elH, 10.6*perCent);
    pancreas->AddElement(elC, 16.9*perCent);
    pancreas->AddElement(elN, 2.2*perCent);
    pancreas->AddElement(elO, 69.4*perCent);
    pancreas->AddElement(elNa, 0.2*perCent);
    pancreas->AddElement(elP, 0.2*perCent);
    pancreas->AddElement(elS, 0.1*perCent);
    pancreas->AddElement(elCl, 0.2*perCent);
    pancreas->AddElement(elK, 0.2*perCent);     
  ...
  // add by Zhaoyang Yuan
  fOriginalMaterials.push_back(resin); //  rho = 
  fOriginalMaterials.push_back(adipose_tissue); //  rho = 
  fOriginalMaterials.push_back(blood_whole); //  rho = 
  fOriginalMaterials.push_back(brain_whole); //  rho = 
  fOriginalMaterials.push_back(breast_calcifications); //  rho = 
  fOriginalMaterials.push_back(breast_mammary_gland); //  rho = 
  fOriginalMaterials.push_back(eye_lens); //  rho = 
  fOriginalMaterials.push_back(gI_tract_intestine); //  rho = 
  fOriginalMaterials.push_back(heart); //  rho = 
  fOriginalMaterials.push_back(heart_blood_filled); //  rho = 
  fOriginalMaterials.push_back(kidney); //  rho = 
  fOriginalMaterials.push_back(liver); //  rho = 
  fOriginalMaterials.push_back(lung); //  rho = 
  fOriginalMaterials.push_back(lymph); //  rho = 
  fOriginalMaterials.push_back(muscle_skeletal); //  rho = 
  fOriginalMaterials.push_back(ovary); //  rho = 
  fOriginalMaterials.push_back(pancreas); //  rho = 
...
void DicomDetectorConstruction::ReadPhantomDataNew()
{
  ...
  G4String mateName;
  G4int nmate;
  // std::cout << __LINE__ << " " << nmate << " " <<  mateName << std::endl;
  for( G4int ii = 0; ii < nMaterials; ii++ ){
    ...
    for( matite = matTab->begin(); matite != matTab->end(); ++matite ) {
      // std::cout << __LINE__ << " " << (*matite)->GetName() << " " <<  mateName << std::endl;
