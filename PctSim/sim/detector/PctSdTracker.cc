...
PctSdTracker::PctSdTracker(const G4String &name, const G4String& hitCollName)
...
PctSdTracker::~PctSdTracker()
{
}

void PctSdTracker::Initialize(G4HCofThisEvent * hce)
{
  // Create hits collection
  fHitsCollection
    = new PctSdTrackerHitsCollection(SensitiveDetectorName, collectionName[0]);

  // Add this collection in hce
  auto hcID
    = G4SDManager::GetSDMpointer()->GetCollectionID(collectionName[0]);
  hce->AddHitsCollection(/* Encoding::CreateCollectionID(Encoding::CID_TRACKER, hcID)*/hcID, fHitsCollection );

}

G4bool PctSdTracker::ProcessHits(G4Step *step,
                             G4TouchableHistory *)
{
    // energy deposit
...
void PctSdTracker::EndOfEvent(G4HCofThisEvent * hce)
{
}
