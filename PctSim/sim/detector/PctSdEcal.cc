...
PctSdEcal::PctSdEcal(const G4String &name, const G4String& hitCollName)
...
void PctSdEcal::Initialize(G4HCofThisEvent * hce)
{
  // Create hits collection
  fHitsCollection
    = new PctSdEcalHitsCollection(SensitiveDetectorName, collectionName[0]);

  // Add this collection in hce
  auto hcID
    = G4SDManager::GetSDMpointer()->GetCollectionID(collectionName[0]);
  hce->AddHitsCollection( /*Encoding::CreateCollectionID(Encoding::CID_ECAL, hcID)*/hcID, fHitsCollection );

}

G4bool PctSdEcal::ProcessHits(G4Step *step,
                             G4TouchableHistory *)
{
    // energy deposit
...
void PctSdEcal::EndOfEvent(G4HCofThisEvent * hce)
{
}
