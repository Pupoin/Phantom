...
TStackingAction::TStackingAction()
...
G4ClassificationOfNewTrack TStackingAction
::ClassifyNewTrack(const G4Track * aTrack)
{
  ...
  if(fStage==0)
  {
    ...
    {
      trackInfo = (TTrackInformation*)(aTrack->GetUserInformation());
...
//             << aTrack->GetPosition() << G4endl;
    ...
    {
      trackInfo = new TTrackInformation(aTrack);
...
G4VHitsCollection* TStackingAction::GetCalCollection()
{
...
void TStackingAction::NewStage()
{
  ...
  else
  {
    ...
    //    G4cout << G4endl;
    TCalorimeterHitsCollection* CHC = 
      (TCalorimeterHitsCollection*)GetCalCollection();
    if(CHC)
    { 
...
void TStackingAction::PrepareNewEvent()
{ 
  fStage = 0; 
}
