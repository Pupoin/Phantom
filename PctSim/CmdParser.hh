...
namespace cli {
	...
	private:
		class CmdBase {
		public:
			explicit CmdBase(const std::string& _name, const std::string& _alternative, const std::string& _description, bool _required, bool _dominant, bool _variadic) :
				name(_name),
				command(_name.size() > 0 ? "-" + _name : ""),
				alternative(_alternative.size() > 0 ? "--" + _alternative : ""),
		...
		public:
			explicit CmdFunction(const std::string& _name, const std::string& _alternative, const std::string& _description, bool _required, bool _dominant) :
				CmdBase(_name, _alternative, _description, _required, _dominant, ArgumentCountChecker<T>::Variadic) {
			}
		...
		public:
			explicit CmdArgument(const std::string& _name, const std::string& _alternative, const std::string& _description, bool _required, bool _dominant) :
				CmdBase(_name, _alternative, _description, _required, _dominant, ArgumentCountChecker<T>::Variadic) {
			}
	...
	public:
        ...
        template <typename T>
        void set_required(const std::string &name, const std::string &alternative, T defaultValue, const std::string &description = "", bool dominant = false)
        {
            auto command = new CmdArgument<T>{name, alternative, description, true, dominant};
    ...
    protected:
        ...
        std::string usage() const
        {
            ...
            for (const auto &command : _commands)
            {
                ...
                if (command->command == std::string("-h"))
                {
                    ss << "\n\n";
                    continue;
                }
                ...
                {
                    // ss << "\t   " << "This parameter is optional. The default value is '" + command->print_value() << "'.";
                    ss << "\t"
                       << "[optional]";
                }
                //ss << "\t"<< command->print_value() <<", "<< command->description;
